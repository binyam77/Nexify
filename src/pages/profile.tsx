...
interface ProfileProps {
  ...
  onStartChat?: (user: { name: string; username: string; photo: string }) => void;
}
...
export default function Profile({ onBackToCommunity, triggerGlobalUpload, onClearGlobalUpload, onStartChat }: ProfileProps) {
      ...
      posts: [
        {
          ...
          description: 'Exploring custom SVG math and real-time canvas renders in React. Super fluid animations under 16ms! #canvas #react #frontend',
          isVideo: false,
          views: 342,
          likes: 48,
          liked: false,
        ...
        },
        {
          ...
          description: 'Designing modular sub-panels and workspace structures for high-density tools. Clean borders, elegant dividers, and smooth transitions! #design #react',
          isVideo: false,
          views: 215,
          likes: 31,
          liked: false,
      ...
      posts: [
        {
          ...
          description: 'Constructing dynamic color tokens for light and dark environments. Balanced contrasts make a huge difference in long coding sessions. #colors #designsystem',
          isVideo: false,
          views: 189,
          likes: 56,
          liked: false,
      ...
      posts: [
        {
          ...
          description: 'A deep dive into distributed Postgres setups. Scaled writes by 3x with simple query isolation patterns. No complexity, pure scale. #postgres #backend',
          isVideo: false,
          views: 512,
          likes: 94,
          liked: false,
        ...
        },
        {
          ...
          description: 'Reducing image build times from 5 minutes to 15 seconds using multi-stage Docker builds and optimal cache layouts. #docker #devops',
          isVideo: false,
          views: 403,
          likes: 72,
          liked: false,
      ...
      posts: [
        {
          ...
          description: 'Crafting beautiful vector patterns that morph dynamically based on screen cursor coordinate tracking. Smooth as silk! #webgl #animation',
          isVideo: false,
          views: 298,
          likes: 85,
          liked: false,
  ...
  const toggleFollowUser = (index: number) => {
    setOtherUsers(prev => prev.map((u, idx) => {
      if (idx === index) {
  ...
  const handleNavigatePost = (direction: 'next' | 'prev') => {
    setSelectedPost(currentPost => {
      ...
      const currentIndex = currentList.findIndex(p => p.id === currentPost.id);
    ...
    if (file) {
      ...
      reader.onload = async (event) => {
        if (event.target?.result) {
          const compressed = await compressImage(event.target.result as string, 0.6, 400);
  ...
  const handleDirectCoverChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = async (event) => {
        if (event.target?.result) {
          const compressed = await compressImage(event.target.result as string, 0.6, 800);
  ...
  const handleSaveProfile = () => {
    ...
    if (!editUsername.trim() || !/^[a-z0-9_]{1,30}$/.test(editUsername.trim().toLowerCase())) {
      alert('Username must be 1-30 alphanumeric characters or underscores only!');
      return;
    }
  ...
  const compressImage = (base64String: string, quality = 0.7, maxWidth = 800): Promise<string> => {
  ...
  const handleThumbnailUploadChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      ...
      reader.onload = async (event) => {
        if (event.target?.result) {
          const compressed = await compressImage(event.target.result as string, 0.5, 300);
  ...
  const handlePostMedia = async () => {
    ...
    try {
      ...
      alert('Unable to publish post. Please check available disk space or try another file.');
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-y-auto bg-gray-50 pb-20 md:pb-6" id="profile-workspace">
      ...
      <header className="sticky top-0 left-0 right-0 h-16 bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-md z-40 flex items-center justify-between px-4 md:px-8 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <img src="/logo.png" alt="Nexify" className="w-8 h-8 object-contain rounded-lg" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
      ...
      <div className="w-full relative shrink-0">
        {viewMode === 'me' ? (
          profile.cover ? (
            ...
            >
              ...
              <div className="relative z-10 flex flex-col items-center text-center">
                ...
                <span className="text-white font-black tracking-tight text-xs sm:text-sm">Personalize Your Cover Space</span>
                <span className="text-white/60 text-[10px] font-bold mt-0.5 max-w-xs">Click here to select and upload your custom brand banner.</span>
              </div>
            </div>
          )
        ) : (
          ...
          <div className="w-full h-40 md:h-52 bg-slate-900 relative overflow-hidden flex flex-col justify-center items-center px-4 border-b border-gray-200/60">
            ...
            <div className="relative z-10 flex flex-col items-center text-center">
              <span className="text-white/20 font-mono tracking-widest text-xs select-none uppercase font-black">ABEL CODES WORKSPACE</span>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-5">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-end gap-3.5 sm:gap-4.5">
              ...
              {viewMode === 'me' ? (
                profile.photo ? (
                  ...
                  <div className="w-full h-full bg-gradient-to-tr from-blue-600 to-indigo-600 flex items-center justify-center text-white text-3xl font-bold tracking-wider">
                    {profile.name ? profile.name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) : 'NX'}
                  </div>
                )
              ) : (
                otherProfile.photo ? (
            ...
            <div className="pt-4 sm:pt-0 sm:pb-1 flex flex-col justify-end">
              <h2 className="text-2xl sm:text-3xl font-black tracking-tight text-gray-950 flex items-center gap-1.5 leading-tight" id="profile-display-name">
          ...
          <div className="flex gap-6 md:gap-8 self-start sm:self-end bg-white border border-gray-100 shadow-md shadow-gray-100/30 px-5 py-3 rounded-2xl">
            <div className="flex flex-col items-center">
              <span className="text-lg font-black text-gray-900 tracking-tight">
                {viewMode === 'me' ? formatCount(followersCount) : formatCount(otherProfile.followersCount)}
              </span>
              <span className="text-xs text-gray-400 font-bold tracking-wide uppercase">Followers</span>
            </div>
            <div className="flex flex-col items-center border-x border-gray-100 px-6 md:px-8">
              <span className="text-lg font-black text-gray-900 tracking-tight">
                {viewMode === 'me' ? formatCount(posts.length) : formatCount(otherPosts.length)}
              </span>
              <span className="text-xs text-gray-400 font-bold tracking-wide uppercase">Posts</span>
            </div>
            <div className="flex flex-col items-center">
              <span className="text-lg font-black text-gray-900 tracking-tight flex items-center gap-1">
                {viewMode === 'me' ? formatCount(starsCount) : formatCount(otherProfile.followingCount)}
              </span>
              <span className="text-xs text-gray-400 font-bold tracking-wide uppercase">Following</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5 mb-6">
          {viewMode === 'me' ? (
            ...
            <>
              ...
              <button 
                onClick={() => onStartChat?.({ name: otherProfile.name, username: otherProfile.username, photo: otherProfile.photo })}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="absolute bottom-2.5 right-2.5 bg-black/60 backdrop-blur-md text-white text-[10px] font-bold px-2 py-1 rounded-lg flex items-center gap-2.5 shadow-sm z-10 group-hover:opacity-0 transition-opacity duration-200 border border-white/5">
                      ...
                      <span className="flex items-center gap-1">
                        <Heart className={`w-3.5 h-3.5 ${post.liked ? 'fill-rose-500 text-rose-500' : 'text-white/95'}`} /> 
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/40 to-transparent flex flex-col justify-end p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3 text-white text-xs font-bold">
                          <span className="flex items-center gap-1"><Eye className="w-3.5 h-3.5 text-white/80" /> {formatCount(post.views)}</span>
                          <span className="flex items-center gap-1"><Heart className={`w-3.5 h-3.5 ${post.liked ? 'fill-rose-500 text-rose-500' : 'text-white/80'}`} /> {formatCount(post.likes)}</span>
                        </div>
        ...
        <div className="fixed inset-0 bg-black/65 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto border border-gray-100 flex flex-col">
            ...
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10 shrink-0">
              <h3 className="text-lg font-black tracking-tight text-gray-900">✏️ Edit Developer Profile</h3>
              <button 
            ...
            <div className="p-6 space-y-5 overflow-y-auto">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <span className="block text-xs font-extrabold tracking-wider text-gray-400 uppercase mb-1">Developer Avatar</span>
                  <button 
                    onClick={() => photoInputRef.current?.click()}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  />
                  <span className="block text-[10px] text-right font-semibold text-gray-400 mt-1">{editName.length}/10</span>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm font-bold text-gray-400 select-none">@</span>
                    <input 
                      type="text" 
                      value={editUsername}
                  ...
                  <span className="block text-[10px] text-right font-semibold text-gray-400 mt-1">{editUsername.length}/30</span>
                </div>
                ...
                <div>
                  ...
                  />
                  <span className="block text-[10px] text-right font-semibold text-gray-400 mt-1">{editBio.length}/150</span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto border border-gray-100 flex flex-col">
            ...
            <div className="p-6 space-y-5 overflow-y-auto">
              ...
              <div className="bg-gray-50 border border-gray-100 p-4 rounded-2xl space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-extrabold tracking-wider text-gray-400 uppercase">Cover Thumbnail (Optional)</span>
                  {uploadThumbnail && (
                    <button 
                ...
                <div className="flex items-center gap-4">
                  ...
                  >
                    {uploadThumbnail ? (
                      <>
                        <img src={uploadThumbnail} alt="Thumbnail preview" className="w-full h-full object-cover" />
                        <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                          <Camera className="w-4 h-4 text-white" />
                        </div>
                      </>
                      ...
                      <div className="text-center p-1 flex flex-col items-center">
                        ...
                        <span className="text-[8px] text-gray-400 font-bold leading-tight">Choose Photo</span>
                      </div>
                    )}
                  </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between text-[11px] font-bold">
                  ...
                  <div className="flex flex-wrap gap-1.5 max-w-[70%]">
                    {(uploadDescription.match(/#\w+/g) || []).map((tag, idx) => (
                      <span key={idx} className="bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full">
                        {tag}
                      </span>
                    ))}
                  </div>
                  <span className="text-gray-400 shrink-0">{uploadDescription.length}/500</span>
                </div>
              </div>
            </div>
      ...
      {selectedPost && (() => {
        ...
        const authorIndex = otherUsers.findIndex(u => u.username === selectedPost.username);
            ...
            <div className="bg-black md:bg-white rounded-none sm:rounded-3xl w-full max-w-[1200px] h-full sm:h-[90vh] md:h-[88vh] overflow-hidden shadow-2xl border border-transparent sm:border-gray-200/50 flex flex-col md:flex-row relative">
              ...
              <aside className="hidden md:flex md:w-[420px] flex-col h-full overflow-hidden bg-white border-l border-gray-200">
                ...
                <header className="flex items-center justify-between p-5 pb-3.5 border-b border-gray-100 shrink-0">
                  ...
                  >
                    <div className="w-11 h-11 rounded-full overflow-hidden border-2 border-blue-600 bg-blue-50 flex items-center justify-center text-blue-600 text-sm font-bold shrink-0 group-hover:scale-105 transition-transform">
                      {postAuthor.photo ? (
                        <img src={postAuthor.photo} alt={postAuthor.name} className="w-full h-full object-cover" />
                      ) : (
                ...
                <section className="flex items-center gap-1 px-5 pb-4 shrink-0 border-b border-gray-100">
                  ...
                  >
                    <Heart className={`w-5 h-5 ${selectedPost.liked ? 'fill-rose-500 text-rose-500' : ''}`} />
                  ...
                  >
                    <Bookmark className={`w-5 h-5 ${selectedPost.saved ? 'fill-amber-500 text-amber-500' : ''}`} />
                ...
                <section className="flex-1 flex flex-col overflow-hidden px-5 pt-3">
                  <h3 className="text-[14px] font-bold text-slate-800 mb-3 flex items-center gap-1.5 shrink-0">
                    Comments <span className="text-xs text-slate-400 font-semibold">{comments.length}</span>
                  </h3>
                  ...
                  <div className="flex-1 overflow-y-auto space-y-4 pb-4 pr-1 scrollbar-thin">
                    {comments.length === 0 ? (
                      <div className="text-center text-slate-400 text-sm py-12 flex flex-col items-center justify-center">
                        ...
                        <p className="text-xs text-slate-400 mt-1">Be the first to share what you think!</p>
                      </div>
                    ) : (
                        ...
                        <div key={comment.id} className="space-y-2">
                          ...
                          <div className="flex gap-2.5 items-start">
                            ...
                            <div className="flex-1 bg-slate-50/90 hover:bg-slate-100 rounded-2xl p-4 border-l-2 border-blue-500 shadow-sm transition-all">
                              ...
                              <p className="text-[13.5px] text-slate-800 leading-relaxed break-words font-medium">{comment.text}</p>
                              
                              <div className="flex items-center gap-4 mt-2 text.5 text-[10px] font-bold text-slate-400">
                                <span>{new Date(comment.timestamp).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}</span>
                                ...
                                <button 
                                  ...
                                  title="Like comment"
                                >
                                  <Heart className={`w-3.5 h-3.5 ${comment.liked ? 'fill-rose-500 text-rose-500' : ''}`} />
                          ...
                          {comment.replies && comment.replies.map((reply) => (
                            <div key={reply.id} className="flex gap-2.5 items-start pl-8">
                              ...
                              <div className="flex-1 bg-slate-100/50 rounded-xl p-2.5">
                                ...
                                <p className="text-xs text-slate-600 leading-normal break-words">{reply.text}</p>
                ...
                <section className="border-t border-gray-100 p-4 pb-5 shrink-0 bg-white relative">
                  <div className="flex items-center gap-2 mb-2 select-none">
                    ...
                    <span className="text-[11px] font-bold text-slate-500">Add comment as you</span>
                  </div>
              ...
              <div className="absolute inset-0 z-20 pointer-events-none md:hidden flex flex-col justify-between">
                ...
                <div className="absolute right-4 bottom-28 flex flex-col gap-4 items-center pointer-events-auto z-30">
                  ...
                  >
                    <div className={`w-11 h-11 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center border border-white/10 ${
                      ...
                      <Heart className={`w-5.5 h-5.5 ${selectedPost.liked ? 'fill-rose-500 text-rose-500' : ''}`} />
                    </div>
                  ...
                  >
                    <div className={`w-11 h-11 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center border border-white/10 ${
                      ...
                      <Bookmark className={`w-5.5 h-5.5 ${selectedPost.saved ? 'fill-amber-400 text-amber-400' : ''}`} />
                    </div>
                    <span className="text-[10px] font-bold text-white mt-1 drop-shadow-md bg-black/25 px-1.5 py-0.5 rounded-full select-none">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 pb-5 bg-gradient-to-t from-black/90 via-black/55 to-transparent pointer-events-auto z-20 flex flex-col gap-2.5">
                  ...
                  <p className="text-[12px] text-white/95 leading-relaxed break-words line-clamp-2 max-h-16 overflow-y-auto pr-2">
                    {selectedPost.description.split(/(\s+)/).map((word, i) => {
                      if (word.startsWith('#')) {
                        return <span key={i} className="text-blue-400 font-bold hover:underline cursor-pointer">{word}</span>;
                      }
                      return <span key={i}>{word}</span>;
                    })}
                  </p>
                        ...
                        comments.map((comment) => (
                          <div key={comment.id} className="space-y-2.5">
                            <div className="flex gap-2.5 items-start">
                             ...
                             {/* Sub-Replies */}
                             {comment.replies && comment.replies.map((reply) => (
                               <div key={reply.id} className="flex gap-2 items-start pl-6">
                                 <div 
                                   onClick={() => {
                                     setMobileCommentsOpen(false);
                                 ...
                                 <div className="flex-1 bg-slate-100/70 rounded-xl py-2 px-3 border-l border-teal-500/20">
                                   ...
                                   <p className="text-[12px] text-slate-700 leading-relaxed break-words font-medium">{reply.text}</p>
          ...
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl max-h-[80vh] overflow-hidden border border-gray-100 flex flex-col">
            ...
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-slate-50 shrink-0">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-black tracking-tight text-gray-900">Explore Developers</h3>
              </div>
              <button 
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <div className={`w-11 h-11 rounded-full bg-gradient-to-tr ${user.gradient} flex items-center justify-center text-white text-sm font-black shadow-inner shrink-0 group-hover:scale-105 transition-transform duration-200`}>
                        {initials}
                      </div>
