...
import { X, Plus, Radio, Upload, Trash2, Camera, Image as ImageIcon } from 'lucide-react';
...
// ቅደም ተከተል፦ Cover → Profile Photo → Name → Bio
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl border border-gray-100 animate-in fade-in zoom-in duration-200">
        ...
        <form onSubmit={handleSubmit} className="max-h-[80vh] overflow-y-auto custom-scrollbar">
          ...
          {/* 1. Cover Photo — full-width banner at the very top */}
          <div
            onClick={() => coverInputRef.current?.click()}
            className="relative w-full h-32 bg-gray-100 border-b border-gray-100 flex items-center justify-center cursor-pointer overflow-hidden group"
          >
            {coverUrl ? (
              <>
          ...
          <div className="p-6 space-y-4">
            ...
            {/* 2. Profile Photo — circular, overlapping-style but inline here since cover is above */}
            <div className="flex items-center gap-4 -mt-12 relative z-10">
              <div className="relative w-20 h-20 rounded-full bg-gray-200 overflow-hidden shrink-0 flex items-center justify-center border-4 border-white shadow-md">
                {avatarUrl ? (
                  ...
                  <div className={`w-full h-full flex items-center justify-center text-white font-black text-lg ${gradients[gradientIndex - 1]?.class || 'bg-gradient-1'}`}>
            ...
            {/* Fallback theme color (only if no avatar uploaded) */}
            {!avatarUrl && (
              <div>
