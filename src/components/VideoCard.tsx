...
export default function VideoCard({ video, currentUser }: VideoCardProps) {
  ...
  const { comments, postComment, deleteComment, editComment, toggleLike: toggleCommentLike, addReply, deleteReply } = useComments(video.id);
  ...
  return (
    ...
    <div className="relative h-full w-full md:flex md:h-full md:items-end md:justify-center md:gap-5 md:px-6 md:py-6">
      ...
      <div className="absolute inset-0 overflow-hidden bg-black md:relative md:h-[92vh] md:max-w-[450px] md:flex-1 md:rounded-xl">
        <video src={video.videoUrl} className="h-full w-full object-cover" loop autoPlay muted playsInline />
        ...
        <div className="absolute bottom-28 left-3 right-20 flex items-center gap-3 rounded-full bg-white/80 px-3.5 py-2 backdrop-blur-md md:bottom-6 md:left-4 md:right-auto md:max-w-[85%]">
          <div className="h-10 w-10 shrink-0 overflow-hidden rounded-full border-2 border-white">
            <img src={video.authorAvatarUrl} alt={video.authorName} className="h-full w-full object-cover" />
          </div>
          <div className="flex min-w-0 flex-col">
            <span className="text-sm font-bold text-slate-900">{video.authorName}</span>
            <p className="mt-0.5 truncate text-[11px] text-slate-700">{video.caption}</p>
          </div>
          {/* Static in the original markup too — no follow logic exists yet */}
          <button type="button" className="ml-auto shrink-0 rounded-full bg-blue-600 px-3 py-1 text-[11px] font-bold text-white hover:bg-blue-700">
            Follow
          </button>
        </div>
      </div>
      ...
      <div className="absolute bottom-32 right-3 z-20 flex flex-col items-center gap-5 md:static md:bottom-auto md:right-auto md:z-auto md:flex-shrink-0 md:gap-4 md:pb-5">
        ...
        />
        <ActionButton
          icon={<MessageCircle size={22} fill={isCommentsOpen ? 'currentColor' : 'none'} />}
        ...
        />
        <button type="button" onClick={share} className="flex h-[46px] w-[46px] items-center justify-center rounded-full bg-white/25 text-white backdrop-blur-sm transition-transform hover:scale-110 md:h-[50px] md:w-[50px] md:bg-blue-100/60 md:text-blue-600 md:backdrop-blur-none" aria-label="Share">
          <Share2 size={20} />
        </button>
        <button type="button" className="flex h-[46px] w-[46px] items-center justify-center rounded-full bg-white/25 text-white backdrop-blur-sm md:h-[50px] md:w-[50px] md:bg-slate-300/80 md:text-slate-700 md:backdrop-blur-none" aria-label="More options">
      ...
      {isCommentsOpen && (
        <CommentModal
          ...
          onPostComment={(text) => postComment(text, currentUser.fullName, currentUser.avatarUrl ?? null)}
          onDeleteComment={deleteComment}
          onEditComment={editComment}
          onToggleLike={toggleCommentLike}
          onAddReply={(id, text) => addReply(id, text, currentUser.fullName, currentUser.avatarUrl ?? null)}
...
function ActionButton({ icon, active, activeColorClass = 'text-rose-400 md:text-rose-500', count, onClick, label }: ActionButtonProps) {
  return (
    <div className="flex flex-col items-center">
      ...
      <span className="mt-1 text-xs font-semibold text-white drop-shadow md:text-slate-700 md:drop-shadow-none">{count}</span>
    </div>
  );
}